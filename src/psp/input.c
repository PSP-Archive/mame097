...
//		{ JOYCODE(0, CODETYPE_AXIS_POS, 1), JOYCODE_1_DOWN },
//
//		if (psp_ctl.analog[PSP_CTRL_ANALOG_Y] == 0xff)	downkey(KEY_2_PAD, &i); // DOWN
//		if (psp_ctl.analog[PSP_CTRL_ANALOG_Y] == 0x00)	downkey(KEY_8_PAD, &i); // UP
//		if (psp_ctl.analog[PSP_CTRL_ANALOG_X] == 0x00)	downkey(KEY_4_PAD, &i); // LEFT
//		if (psp_ctl.analog[PSP_CTRL_ANALOG_X] == 0xff)	downkey(KEY_6_PAD, &i); // RIGHT
		if (0 ==joynum)
		{
			if (keyorientation) {
				if( codetype == CODETYPE_AXIS_POS ){
					...
					else{	return (0x20 >=psp_ctl.Lx);}
				}else{
					if (0 ==joyindex)
					{		return (0x20 >=psp_ctl.Ly);}
					else{	return (0xE0 <=psp_ctl.Lx);}
				}
			} else {
				if( codetype == CODETYPE_AXIS_POS ){
					if (0 ==joyindex)
					{		return (0xE0 <=psp_ctl.Lx);}
					else{	return (0xE0 <=psp_ctl.Ly);}
				}else{
					if (0 ==joyindex)
					{		return (0x20 >=psp_ctl.Lx);}
					else{	return (0x20 >=psp_ctl.Ly);}
				}
			}
		}
		return 0;
...
static void downkey(int ikey, int *idx) {
	key[ikey] =1;
	keytmp[*idx] =ikey;
	*idx +=1;
}
