...
you installed DOS/GCC & typed the ...

DOS>gcc -O1 -march=pentium m68kmake.c -o m68kmake
...
================================================
  ÇoÇrÇoäJî≠ócítâÄ  http://ameblo.jp/pspdevblog/  Åiñ{äŸÅjÅiÉuÉçÉOÅj
  ÇoÇrÇoäJî≠ócítâÄ  http://psp.nukenin.jp/        Åiï äŸÅjÅiÇgÇoÅj
================================================
...
typedef struct
{
	...
	char mpu_mode;							/* User or supervisor mode */
	char mpus/*[1 NUM_MPUS+1]*/; 			/* Allowed MPUs */
	char if_safety_dummy;
	unsigned char cycles;					/* cycles for 000, 010, 020 */
} opcode_struct;
...
//int get_oper_cycles000(opcode_struct* op, int ea_mode);//, int mpu_type);
...
const static char* g_cc_table[16][2] =
{
	{ "t",	"T"}, /* 0000 */
	{ "f",	"F"}, /* 0001 */
	{"hi", "HI"}, /* 0010 */
...
static int g_ea_cycle_table[13]/*[NUM_MPUS]*/[3] =
{/* 	  000	  */
	{ 0,  0,  0}, /* EA_MODE_NONE */
	{ 0,  4,  8}, /* EA_MODE_AI   */
	{ 0,  4,  8}, /* EA_MODE_PI   */
...
int get_oper_cycles000(opcode_struct* op, int ea_mode )
{
	int size = g_size_select_table[op->size];

	if(op->mpus/*[0 mpu_type]*/ == '.')
		return 0;

//	if(mpu_type < MPU_TYPE_020)
	{
...
(ea_mode == EA_MODE_I || ea_mode == EA_MODE_NONE) && op->size == 32 &&
		   ((strcmp(op->name, "add")  == 0 && strcmp(op->spec_proc, "er") == 0) ||
			...
			return op->cycles + g_ea_cycle_table[ea_mode]/*[mpu_type]*/[size] + 2;

		if(strcmp(op->name, "jmp") == 0)	return op->cycles + g_jmp_cycle_table[ea_mode];
		if(strcmp(op->name, "jsr") == 0)	return op->cycles + g_jsr_cycle_table[ea_mode];
		if(strcmp(op->name, "lea") == 0)	return op->cycles + g_lea_cycle_table[ea_mode];
		if(strcmp(op->name, "pea") == 0)	return op->cycles + g_pea_cycle_table[ea_mode];
		if(strcmp(op->name, "movem") == 0)	return op->cycles + g_movem_cycle_table[ea_mode];
	}
	return op->cycles + g_ea_cycle_table[ea_mode]/*[mpu_type]*/[size];
}
...
void generate_opcode_ea_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op)
{
	...
	replace->length = old_length;
	if(HAS_EA_DI(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_DI);
	replace->length = old_length;
	if(HAS_EA_IX(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_IX);
	replace->length = old_length;
	if(HAS_EA_AW(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_AW);
	replace->length = old_length;
	if(HAS_EA_AL(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_AL);
	replace->length = old_length;
	if(HAS_EA_PCDI(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_PCDI);
	replace->length = old_length;
	if(HAS_EA_PCIX(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_PCIX);
	replace->length = old_length;
	if(HAS_EA_I(op->ea_allowed))		generate_opcode_handler(filep, body, replace, op, EA_MODE_I);
	replace->length = old_length;
}
...
void populate_table(void)
{
	...
	for(op = g_opcode_input_table;;op++)
	{
		...
		/* MPU operating mode (U = user or supervisor, S = supervisor only */
		ptr += skip_spaces(ptr);
//		for(i=0;i<NUM_MPUS;i++)
		...
		/* Allowed MPUs for this instruction */
//		for(i=0;i<NUM_MPUS;i++)
		{
			ptr += skip_spaces(ptr);
			if(*ptr == UNSPECIFIED_CH)
			{
				op->mpus/*[0 i]*/ = UNSPECIFIED_CH;
				op->cycles = 0;
				ptr++;
			}
			else
...
int main(int argc, char **argv)
{
	...
	printf("\n\tMusashi 68000 emulator. 'M44+' @MAME106s_based. automaker. \n" );
	printf("\t(C)1998-2000 Karl Stenerud. (karl@mame.net)\n");
	printf("\t(C)2006 PSP battle nursery scrool.\n\n");
	...
	if((g_input_file	=fopen(g_input_filename, "rt")) == NULL)perror_exit("can't open %s for input", g_input_filename);
	...
	for(;;)
	{
		...
		{
			if(prototype_header_read)	error_exit("Duplicate prototype header");
		...
		{
			if(table_header_read)		error_exit("Duplicate table header");
			read_insert(temp_insert);
		...
		{
			if(ophandler_header_read)	error_exit("Duplicate opcode handler header");
			read_insert(temp_insert);
		...
		{
			if(prototype_footer_read)	error_exit("Duplicate prototype footer");
			read_insert(prototype_footer_insert);
			prototype_footer_read = 1;
		}
		else if(strcmp(section_id, ID_TABLE_FOOTER) == 0)
		{
			if(table_footer_read)		error_exit("Duplicate table footer");
			read_insert(table_footer_insert);
			table_footer_read = 1;
		}
		else if(strcmp(section_id, ID_OPHANDLER_FOOTER) == 0)
		{
			if(ophandler_footer_read)	error_exit("Duplicate opcode handler footer");
			read_insert(ophandler_footer_insert);
			ophandler_footer_read = 1;
		}
		else if(strcmp(section_id, ID_TABLE_BODY) == 0)
		{
			if(!prototype_header_read)	error_exit("Table body encountered before prototype header");
			if(!table_header_read)		error_exit("Table body encountered before table header");
			if(!ophandler_header_read)	error_exit("Table body encountered before opcode handler header");

			if(table_body_read)			error_exit("Duplicate table body");
		...
		{
			if(!prototype_header_read)	error_exit("Opcode handlers encountered before prototype header");
			if(!table_header_read)		error_exit("Opcode handlers encountered before table header");
			if(!ophandler_header_read)	error_exit("Opcode handlers encountered before opcode handler header");
			if(!table_body_read)		error_exit("Opcode handlers encountered before table body");

			if(ophandler_body_read)		error_exit("Duplicate opcode handler section");
