 ...
 ***************************************************************/
#define LD_A_R {												\
	zA = ((zA+7) & 0xff);/*A = (R & 0x7f) | R2;*/					\
	F = (F & CF) | SZ[zA] | (zIFFS&IFF2_ON)/*( zIFF2 << 2 )*/;						\
}
...
#define LDI {													\
	...
	WM( DE, io );												\
	F &= SF | ZF | CF;											\
	if( (zA + io) & 0x02 ) F |= YF; /* bit 1 -> flag 5 */		\
	if( (zA + io) & 0x08 ) F |= XF; /* bit 3 -> flag 3 */		\
	HL++; DE++; BC--;											\
	if( BC ) F |= VF;											\
}
...
#define CPI {													\
	...
	UINT8 res = zA - val;										\
	HL++; BC--;													\
	F = (F & CF) | (SZ[res]&~(YF|XF)) | ((zA^val^res)&HF) | NF;	\
	if( F & HF ) res -= 1;										\
...
OP(xycb,07) { zA = RLC( RM(EA) ); WM( EA,zA );					} /* RLC  A=(XY+o)    */
...
OP(xycb,0f) { zA = RRC( RM(EA) ); WM( EA,zA );					} /* RRC  A=(XY+o)    */
...
OP(xycb,17) { zA = RL( RM(EA) ); WM( EA,zA );						} /* RL   A=(XY+o)    */
...
OP(xycb,1f) { zA = RR( RM(EA) ); WM( EA,zA );						} /* RR   A=(XY+o)    */
...
OP(xycb,27) { zA = SLA( RM(EA) ); WM( EA,zA );					} /* SLA  A=(XY+o)    */
...
OP(xycb,2f) { zA = SRA( RM(EA) ); WM( EA,zA );					} /* SRA  A=(XY+o)    */
...
OP(xycb,37) { zA = SLL( RM(EA) ); WM( EA,zA );					} /* SLL  A=(XY+o)    */
...
OP(xycb,3f) { zA = SRL( RM(EA) ); WM( EA,zA );					} /* SRL  A=(XY+o)    */
...
OP(xycb,87) { zA = RES(0, RM(EA) ); WM( EA,zA );					} /* RES  0,A=(XY+o)  */
...
OP(xycb,8f) { zA = RES(1, RM(EA) ); WM( EA,zA );					} /* RES  1,A=(XY+o)  */
...
OP(xycb,97) { zA = RES(2, RM(EA) ); WM( EA,zA );					} /* RES  2,A=(XY+o)  */
...
OP(xycb,9f) { zA = RES(3, RM(EA) ); WM( EA,zA );					} /* RES  3,A=(XY+o)  */
...
OP(xycb,a7) { zA = RES(4, RM(EA) ); WM( EA,zA );					} /* RES  4,A=(XY+o)  */
...
OP(xycb,af) { zA = RES(5, RM(EA) ); WM( EA,zA );					} /* RES  5,A=(XY+o)  */
...
OP(xycb,b7) { zA = RES(6, RM(EA) ); WM( EA,zA );					} /* RES  6,A=(XY+o)  */
...
OP(xycb,bf) { zA = RES(7, RM(EA) ); WM( EA,zA );					} /* RES  7,A=(XY+o)  */
...
OP(xycb,c7) { zA = SET(0, RM(EA) ); WM( EA,zA );					} /* SET  0,A=(XY+o)  */
...
OP(xycb,cf) { zA = SET(1, RM(EA) ); WM( EA,zA );					} /* SET  1,A=(XY+o)  */
...
OP(xycb,d7) { zA = SET(2, RM(EA) ); WM( EA,zA );					} /* SET  2,A=(XY+o)  */
...
OP(xycb,df) { zA = SET(3, RM(EA) ); WM( EA,zA );					} /* SET  3,A=(XY+o)  */
...
OP(xycb,e7) { zA = SET(4, RM(EA) ); WM( EA,zA );					} /* SET  4,A=(XY+o)  */
...
OP(xycb,ef) { zA = SET(5, RM(EA) ); WM( EA,zA );					} /* SET  5,A=(XY+o)  */
...
OP(xycb,f7) { zA = SET(6, RM(EA) ); WM( EA,zA );					} /* SET  6,A=(XY+o)  */
...
OP(xycb,ff) { zA = SET(7, RM(EA) ); WM( EA,zA );					} /* SET  7,A=(XY+o)  */
...
OP(ed,78) { zA = IN(BC); F = (F & CF) | SZP[zA];					} /* IN   E,(C)       */
OP(ed,79) { OUT(BC, zA);											} /* OUT  (C),zA       */
...
OP(op,2f) { zA ^= 0xff; F = (F&(SF|ZF|PF|CF))|HF|NF|(zA&(YF|XF));	} /* CPL              */

OP(op,30) { JR_COND( !(F & CF), 0x30 );							} /* JR   NC,o        */
OP(op,31) { SP = ARG16();										} /* LD   SP,w        */
OP(op,32) { EA = ARG16(); WM( EA, zA );							} /* LD   (w),zA       */
OP(op,33) { SP++;												} /* INC  SP          */
OP(op,34) { WM( HL, INC(RM(HL)) );								} /* INC  (HL)        */
OP(op,35) { WM( HL, DEC(RM(HL)) );								} /* DEC  (HL)        */
OP(op,36) { WM( HL, ARG() );									} /* LD   (HL),n      */
OP(op,37) { F = (F & (SF|ZF|PF)) | CF | (zA & (YF|XF));			} /* SCF              */

OP(op,38) { JR_COND( F & CF, 0x38 );							} /* JR   C,o         */
OP(op,39) { ADD16(hl, sp);										} /* ADD  HL,SP       */
OP(op,3a) { EA = ARG16(); zA = RM( EA );							} /* LD   A,(w)       */
OP(op,3b) { SP--;												} /* DEC  SP          */
OP(op,3c) { zA = INC(zA);											} /* INC  zA           */
OP(op,3d) { zA = DEC(zA);											} /* DEC  zA           */
OP(op,3e) { zA = ARG();											} /* LD   A,n         */
OP(op,3f) { F = ((F&(SF|ZF|PF|CF))|((F&CF)<<4)|(zA&(YF|XF)))^CF;	} /* CCF              */
...
OP(op,77) { WM( HL, zA );										} /* LD   (HL),zA      */

OP(op,78) { zA = B;												} /* LD   A,B         */
OP(op,79) { zA = C;												} /* LD   A,C         */
OP(op,7a) { zA = D;												} /* LD   A,D         */
OP(op,7b) { zA = E;												} /* LD   A,E         */
OP(op,7c) { zA = H;												} /* LD   A,H         */
OP(op,7d) { zA = L;												} /* LD   A,L         */
OP(op,7e) { zA = RM(HL);											} /* LD   A,(HL)      */
//OP(op,7f) {														} /* LD   A,zA         */
...
OP(op,d3) { unsigned n = ARG() | (zA << 8); OUT( n, zA );			} /* OUT  (n),zA       */
...
OP(op,db) { unsigned n = ARG() | (zA << 8); zA = IN( n );			} /* IN   A,(n)       */
...
OP(op,f3) { /*zIFF1 = zIFF2 = 0;*/	zIFFS=0;					} /* DI               */
...
//	if( zIFF1 )
	if( (zIFFS&IFF1_ON) )
	{
		...
		if( IM == 2 )
		{
			...
			/* CALL opcode timing */
			zEXcyc += 17;//(17)//cc[Z80_TABLE_op][0xcd];
		}
		else
		...
		if( IM == 1 )
		{
			...
			zEXcyc += 13;//(11+2)//cc[Z80_TABLE_op][0xff] + cc[Z80_TABLE_ex][0xff];
		}
		else
		{
			...
			switch (irq_vector & 0xff0000)
			{
				case 0xcd0000:	/* call */
					...
					zEXcyc += 19;//(17+2)//cc[Z80_TABLE_op][0xcd] + cc[Z80_TABLE_ex][0xff];
					break;
				case 0xc30000:	/* jump */
					PCD = irq_vector & 0xffff;
					/* JP $xxxx + 2 cycles */
					zEXcyc += 12;//(10+2)//cc[Z80_TABLE_op][0xc3] + cc[Z80_TABLE_ex][0xff];
					break;
				default:		/* rst (or other opcodes?) */
					PUSH( pc );
					PCD = irq_vector & 0x0038;
					/* RST $xx + 2 cycles */
					zEXcyc += cc[Z80_TABLE_op][PCD] + cc[Z80_TABLE_ex][PCD];
					break;
			}
		}
		change_pc(PCD);
...
//	state_save_register_UINT8("z80", cpu, "IFF2", &Z80.iff2, 1);
	state_save_register_UINT8("z80", cpu, "IFFS", &Z80.iffs, 1);
